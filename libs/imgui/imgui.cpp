...
struct OcornutImguiContext {
  void render(ImDrawData *_drawData) {
      ...
      for (ImTextureData *texData : *_drawData->Textures) {
        ...
        case ImTextureStatus_WantCreate: {
          ImGui::TextureBgfx tex = {
              .handle = bgfx::createTexture2D((uint16_t)texData->Width,
                                              (uint16_t)texData->Height, false,
          ...
          bgfx::updateTexture2D(
              tex.handle, 0, 0, 0, 0, bx::narrowCast<uint16_t>(texData->Width),
        ...
        case ImTextureStatus_WantUpdates: {
          ...
          for (ImTextureRect &rect : texData->Updates) {
            ...
            bgfx::updateTexture2D(tex.handle, 0, 0, rect.x, rect.y, rect.w,
                                  rect.h, pix);
          }
        } break;
    ...
    const ImVec2 clipScale =
        _drawData->FramebufferScale; // (1,1) unless using retina display which
                                     // are often (2,2)
    ...
    for (int32_t ii = 0, num = _drawData->CmdListsCount; ii < num; ++ii) {
      ...
      for (const ImDrawCmd *cmd = drawList->CmdBuffer.begin(),
        ...
        if (cmd->UserCallback) {
          ...
          if (clipRect.x < dispWidth && clipRect.y < dispHeight &&
  ...
  void destroy() {
    ...
    for (ImTextureData *tex : ImGui::GetPlatformIO().Textures) {
      const bgfx::TextureHandle t =
          bgfx::TextureHandle{.idx = (uint16_t)tex->GetTexID()};
      bgfx::destroy(t);
    }
...
extern "C" {
IMGUI_IMPL_API void ImGui_ImplBgfx_Init() { s_ctx.create(0, nullptr); }

IMGUI_IMPL_API void ImGui_ImplBgfx_Shutdown() { s_ctx.destroy(); }

IMGUI_IMPL_API void ImGui_ImplBgfx_NewFrame(bgfx::ViewId _viewId) {
  s_ctx.beginFrame(_viewId);
}

IMGUI_IMPL_API void ImGui_ImplBgfx_RenderDrawData() { s_ctx.endFrame(); }
}
